...
```
``` r
git clone https://github.com/DAV_NJB/DAV_NJB
```
...
```
``` r
colcon build --packages-select DAV_NJB --symlink-install
```
...
``` r
ros2 launch DAV_NJB launch_example1.launch.py
```
...
- `DAV_NJB` >> `cool_ros2_package` (the folder was already renamed after `Use this template`)
- `DAV_NJB` >> `mycoolusername`
